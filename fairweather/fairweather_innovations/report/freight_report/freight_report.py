...
def get_items(filters, additional_query_columns):
	...
	freight_item = frappe.db.get_single_value("System Settings", "freight_item")
	...
	return frappe.db.sql("""
		...
		where `tabSales Invoice`.name = `tabSales Invoice Item`.parent
			and `tabSales Invoice Item`.item_code = '{1}'
			and `tabSales Invoice`.docstatus = 1 %s %s
		order by `tabSales Invoice`.posting_date desc, `tabSales Invoice Item`.item_code desc
		""".format(additional_query_columns or '', freight_item or '') % (conditions, match_conditions), filters, as_dict=1)
