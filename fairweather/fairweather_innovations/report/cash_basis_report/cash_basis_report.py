...
def _execute(filters, additional_table_columns=None, additional_query_columns=None):
    ...
    for inv in invoice_list:
        # invoice details
        sales_order = list(set(invoice_so_dn_map.get(
            inv.name, {}).get("sales_order", [])))
        delivery_note = list(set(invoice_so_dn_map.get(
            inv.name, {}).get("delivery_note", [])))
        cost_center = list(set(invoice_cc_wh_map.get(
            inv.name, {}).get("cost_center", [])))
        warehouse = list(set(invoice_cc_wh_map.get(
            inv.name, {}).get("warehouse", [])))
        ...
        row += [
            ...
            ", ".join(sales_order), ", ".join(
                delivery_note), ", ".join(cost_center),
            ", ".join(warehouse), company_currency
        ]
        # map income values
        base_net_total = 0
        for income_acc in income_accounts:
            income_amount = flt(invoice_income_map.get(
                inv.name, {}).get(income_acc))
        ...
        for tax_acc in tax_accounts:
            if tax_acc not in income_accounts:
                tax_amount = flt(invoice_tax_map.get(
                    inv.name, {}).get(tax_acc))
                total_tax += tax_amount
                row.append(tax_amount)

        # total tax, grand total, outstanding amount & rounded total
        row += [total_tax, inv.base_grand_total,
                inv.base_rounded_total, inv.outstanding_amount]
...
def get_columns(invoice_list, additional_table_columns):
    ...
    columns += [
        _("Customer Group") + ":Link/Customer Group:120", _("Territory") +
        ":Link/Territory:80",
        _("Tax Id") + "::80", _("Receivable Account") +
        ":Link/Account:120", _("Mode of Payment") + "::120",
        _("Project") + ":Link/Project:80", _("Owner") +
        "::150", _("Remarks") + "::150",
        _("Sales Order") + ":Link/Sales Order:100", _("Delivery Note") +
        ":Link/Delivery Note:100",
        _("Cost Center") + ":Link/Cost Center:100", _("Warehouse") +
        ":Link/Warehouse:100",
        {
            "fieldname": "currency",
            "label": _("Currency"),
    ...
    income_columns = [(account + ":Currency/currency:120")
    ...
    for account in tax_accounts:
        if account not in income_accounts:
            ...
            tax_columns.append(acc + ":Currency/currency:120")
...
def get_invoice_income_map(invoice_list):
    ...
    for d in income_details:
        invoice_income_map.setdefault(
            d.parent, frappe._dict()).setdefault(d.income_account, [])
    ...
    for d in tax_details:
        if d.description in income_accounts:
            if invoice_income_map[d.parent].has_key(d.description):
                invoice_income_map[d.parent][d.description] += flt(
                    d.tax_amount)
            else:
                invoice_income_map[d.parent][d.description] = flt(d.tax_amount)
        else:
            invoice_tax_map.setdefault(
                d.parent, frappe._dict()).setdefault(d.description, [])
